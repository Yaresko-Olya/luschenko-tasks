...
function t9() {
    ...
    const temp = [];
    a9.forEach(elem => temp.push(elem.toLowerCase()))
    a9 = temp;
    return a9;
}
...
// С помощью filter, переберите массив a11 и создайте новый массив a11_res куда добавьте только те элементы массива 
...
function t14() {
    const temp = [];
    a14.forEach(elem => temp.push(elem.toLowerCase()))
    a14 = temp;
    
    let res = temp.includes(a14_sym.toLowerCase());
    return res;
...
// Task 15 ============================================
/*  Дан массив a15. С помощью метода filter функция t15 должна создать на его основе массив a15_res содержащий записи только тех 
// пользователей, номер паспорта (pnum) которых не пустая строка и длина номера - 6 символов. 
