...
let div2 = document.querySelector('.div-3_2');
let out = document.querySelector('.out-3');

function t3() {
    div1.addEventListener('touchstart', () => {
        out.textContent = 'div-3_1';
    });
    div2.addEventListener('touchstart', () => {
        out.textContent = 'div-3_2';
    });
}
...
function t8() {
    let randomIndex = Math.floor(Math.random() * a8.length);
  
    let randomColor = a8[randomIndex];
...
function t11(event) {
    ...
    document.querySelector('.out-11').textContent = `x = ${event.touches[0].radiusX} y = ${event.touches[0].radiusY}`
}
...
function nextFunction() {
    ...
    let activeImg = images[count];
    activeImg.classList.add('active-img');
    document.querySelector('.img-12-max').src = activeImg.src;
}
