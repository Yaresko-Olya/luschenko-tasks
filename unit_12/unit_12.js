...
// Функция должна вывести в out-7 четные значения из массива a7. Как видите, вложенные массивы имеют разную длину, значит, предыдущий вариант уже не работает. Используем цикл. Вывод - через пробел.
...
function f15() {
    ...
    for (let i = 0; i < a15.length; i++) {
        if (a15[i].length > len.length) {
            len = a15[i];
        }
    }
    maxLenArr = len.length;

    document.querySelector('.out-15').innerHTML = maxLenArr;
}
...
// который соответствует всем условиям приведенным ниже 
